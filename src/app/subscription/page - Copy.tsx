...
/** Format renewal/invoice date (backend sends the actual renewal day, e.g. Mar 9). */
function formatRenewalOrInvoiceDate(isoEndsAt: string | null | undefined): string {
  if (!isoEndsAt) return '';
  const d = new Date(isoEndsAt);
  return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate())).toLocaleDateString(undefined, { timeZone: 'UTC' });
}
