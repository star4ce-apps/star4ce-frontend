...
export default function DealershipRequestsPage() {
  const [pendingRequests, setPendingRequests] = useState<DealershipAccessRequest[]>([]);
  const [processedRequests, setProcessedRequests] = useState<DealershipAccessRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [userRole, setUserRole] = useState<string | null>(null);
  const [showRejectModal, setShowRejectModal] = useState(false);
  const [selectedRequest, setSelectedRequest] = useState<DealershipAccessRequest | null>(null);
  ...
  async function handleReject(requestId: number, notes: string) {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/dealership-requests/${requestId}/reject`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
          ...
          <div className="mb-8">
            ...
            {pendingRequests.length === 0 ? (
              <div className="rounded-xl p-8 text-center" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB' }}>
                <p className="text-base" style={{ color: '#9CA3AF' }}>No pending requests</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr style={{ backgroundColor: '#F9FAFB' }}>
                      <th className="text-left py-4 px-6 text-sm font-semibold" style={{ color: '#6B7280' }}>Corporate User</th>
                    ...
                    {pendingRequests.map((request) => (
                      <tr key={request.id} style={{ borderTop: '1px solid #E5E7EB' }}>
                        <td className="py-4 px-6">
                          <span className="text-base font-medium" style={{ color: '#232E40' }}>{request.corporate_user_email}</span>
                        </td>
                        <td className="py-4 px-6">
          ...
          {showRejectModal && selectedRequest && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="rounded-xl p-6 max-w-md w-full mx-4" style={{ backgroundColor: '#FFFFFF' }}>
                <h2 className="text-2xl font-bold mb-4" style={{ color: '#232E40' }}>Reject Access Request</h2>
                <p className="text-sm mb-4" style={{ color: '#6B7280' }}>
                  Rejecting access request from <strong>{selectedRequest.corporate_user_email}</strong> for <strong>{selectedRequest.dealership_name}</strong>
                </p>
                <div className="mb-4">
